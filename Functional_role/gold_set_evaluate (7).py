...
def build_fractional_confusion_matrix(testable: pd.DataFrame, labels: list) -> np.ndarray:
    """
    Builds a (true x predicted) confusion matrix that handles multi-label
    textbook ground truth WITHOUT dropping any of the listed acceptable
    roles and WITHOUT an arbitrary bias toward whichever role happens to
    be listed first.

    For a row with acceptable truths T = {t1, ..., tk} and prediction p:
      - if p is IN T          -> add 1.0 to cell (true=p, pred=p)
                                  (full credit, same notion of "correct"
                                  your accuracy number already uses)
      - if p is NOT in T      -> add 1/k to cell (true=ti, pred=p) for
                                  EVERY ti in T (the miss is split evenly
                                  across all acceptable roles, since the
                                  textbook didn't commit to just one)

    This keeps every row's total mass = 1.0, so trace(C)/C.sum() reproduces
    the exact same accuracy already printed above, while still giving you
    a real confusion matrix, precision/recall/F1, and MCC without silently
    discarding any of the acceptable ground-truth roles.
    """
    ...
    for _, row in testable.iterrows():
        ...
        if pred in truths:
            ...
            share = 1.0 / len(truths)
            for t in truths:
                C[idx[t], j] += share
    return C


def multiclass_mcc_from_confusion(C: np.ndarray) -> float:
    """
    Generalized (Gorodkin 2004) multiclass MCC computed directly from a
    confusion matrix. Works identically for integer or fractional-credit
    matrices -- it's pure algebra over row/column sums, so the fractional
    credit from multi-label ground truth flows through correctly instead
    of needing to be forced into single y_true/y_pred label arrays first.
    """
    s = C.sum()
    if s == 0:
        return 0.0
    c = np.trace(C)
    t_k = C.sum(axis=1)  # true-class totals (row sums)
    p_k = C.sum(axis=0)  # predicted-class totals (column sums)
...
def print_and_save_metrics(testable: pd.DataFrame):
    """
    Prints + saves, all computed from the fractional-credit confusion
    matrix so multi-label textbook ground truth is never collapsed or
    dropped:
      ...
      - full confusion matrix (rows = textbook truth, cols = predicted),
        ...
        (values may be non-integer where a row's miss was split across
        multiple acceptable truth roles -- see the note printed below)
    """
    if len(testable) == 0:
        print("\n[METRICS] No testable rows -- nothing to compute.")
        return

    n_multilabel = int((testable["textbook_role_canonical"].str.contains(r"\|")).sum())

    all_truth_labels = set()
    for truths in testable["textbook_role_canonical"].str.split("|"):
        all_truth_labels.update(truths)
    labels = sorted(all_truth_labels | set(testable["predicted_role"].tolist()))

    C = build_fractional_confusion_matrix(testable, labels)

    support = C.sum(axis=1)          # true-class totals
    predicted_total = C.sum(axis=0)  # predicted-class totals
    tp = np.diag(C)

    with np.errstate(divide="ignore", invalid="ignore"):
        recall = np.where(support > 0, tp / support, 0.0)
        precision = np.where(predicted_total > 0, tp / predicted_total, 0.0)
        f1 = np.where((precision + recall) > 0,
                       2 * precision * recall / (precision + recall), 0.0)

    accuracy = tp.sum() / C.sum()
    present = support > 0
    balanced_accuracy = recall[present].mean() if present.any() else 0.0
    mcc = multiclass_mcc_from_confusion(C)
    ...
    if n_multilabel:
        print(f"[NOTE] {n_multilabel}/{len(testable)} testable rows had multiple acceptable "
              f"textbook roles. A correct-either-way match still gets full credit on that "
              f"role's diagonal cell; a genuine miss is split EVENLY across every listed "
              f"acceptable role (not just the first one), so no acceptable ground-truth "
              f"role is ever dropped. This means some confusion-matrix / support values "
              f"below are fractional rather than whole counts.")
    print(f"\nAccuracy:           {accuracy:.4f}")
    print(f"Balanced accuracy:  {balanced_accuracy:.4f}  (per-class recall averaged -- robust to class imbalance)")
    ...
    weighted_p = np.dot(precision, support) / total_support if total_support else 0.0
    weighted_r = np.dot(recall, support) / total_support if total_support else 0.0
    weighted_f1 = np.dot(f1, support) / total_support if total_support else 0.0

    print(f"\nPer-class (sorted by support):")
    print(per_class.to_string(index=False, formatters={
        "precision": "{:.3f}".format, "recall": "{:.3f}".format,
        "f1": "{:.3f}".format, "support": "{:.2f}".format}))
    ...
    cm_df = pd.DataFrame(C, index=[f"true:{l}" for l in labels],
                          columns=[f"pred:{l}" for l in labels]).round(3)
